...
const { zipsForState } = require('./zipdata');
...
async function step1(page, task, data, proxy, tag) {
  ...
  // Postal code consistent with the proxy's geo state (falls back to full list).
  const state = proxy.geo && proxy.geo.state;
  const postalCode = randomFrom(zipsForState(data.postalCodes, state));

  console.log(`  [${tag}] ${firstName} ${lastName} | ${task.email} | ${postalCode}${state ? ` (${state})` : ''}`);
